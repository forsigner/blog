...
异步数据管理一直是一个重点和难点，可以这么说，80%的 web 应用会有异步请求数据并在 UI 中消费，并且在相当多的 web 应用中，处理异步数据是它的核心业务逻辑。
...
个人认为，异步数据不应该使用状态管理来维护，应该放在组件内。对于大多数 web 应用，状态管理中的数据应该是比较薄的一层，并且应该避免在状态管理中处理异步带来的附作用。也许，Redux 默认不支持处理异步数据，是一个相当有远见的决定。
